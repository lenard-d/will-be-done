...
describe("habit backup compatibility", () => {
  ...
  it("roundtrips habits, routines, and explicit completions", () => {
    ...
    syncDispatch(source, loadSpaceBackup({ backup: populatedBackup() }));
  ...
  it("preserves habit tables when legacy backup sections are absent", () => {
    const db = createDB();
    syncDispatch(db, loadSpaceBackup({ backup: populatedBackup() }));
    ...
    expect(restored.habits).toEqual(populatedBackup().habits);
    expect(restored.routines).toEqual(populatedBackup().routines);
    expect(restored.habitCompletions).toEqual(
      populatedBackup().habitCompletions,
    );
  });

  it("clears habit tables when sections are explicitly empty", () => {
    const db = createDB();
    syncDispatch(db, loadSpaceBackup({ backup: populatedBackup() }));
