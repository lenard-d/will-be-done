...
describe("space backup compatibility", () => {
  ...
  it("roundtrips task and template nature without changing it", () => {
    const source = new DB(new BptreeInmemDriver(), {
      traits: [dbIdTrait("space", "a0000000-0000-4000-8000-000000000001")],
    });
    execSync(source.loadTables(registeredSpaceSyncableTables));
    const backup: Backup = {
      ...baseBackup,
      projects: [
        {
    ...
    syncDispatch(source, loadSpaceBackup({ backup }));
    const exported = selectSync(source, { selector: getSpaceBackup, args: {} });
    const restored = new DB(new BptreeInmemDriver(), {
      traits: [dbIdTrait("space", "a0000000-0000-4000-8000-000000000002")],
    });
    execSync(restored.loadTables(registeredSpaceSyncableTables));
    ...
    expect(roundtripped.tasks[0]?.nature).toBe("red");
    expect(roundtripped.taskTemplates[0]?.nature).toBe("green");
  });
});
